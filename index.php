<?php
// Database Connection
$conn = new mysqli("127.0.0.1", "root", "", "health_db");
    ...
    <div class="card">
        ...
        if(isset($_POST['analyze'])){
            ...
            if(move_uploaded_file($_FILES["xray"]["tmp_name"], $target_file)){
                ...
                $output = shell_exec("python src/predict_single.py $escaped_path 2>&1");
                ...
                if(isset($result['label'])){
                    ...
                    (patient_name, prediction, confidence, image_path, gradcam_path) 
                    VALUES ('$p_name', '$label', '$conf', '$target_file', '$gradcam')";

                    if(!$conn->query($sql)){
                        die("Database Error: " . $conn->error);
                    }
                    
                    echo "<div class='result-box' style='background:".($label=='NORMAL'?'#d1fae5':'#fee2e2')."'>";
                    echo "<strong>Result: $label</strong> (" . number_format($conf, 1) . "% confidence)";
                    echo "</div>";

                    echo "<img src='$gradcam' style='width:100%; margin-top:15px; border-radius:10px;'>";
                } else {
                    echo "<div style='color:red;'>Error: ".$output."</div>";
                }
            }
        }
        ?>
            ...
            <tbody>
                ...
                while($row = $res->fetch_assoc()){
                    echo "<tr>
                        ...
                        <td><a href='generate_report.php?id={$row['id']}'>📥 Download</a></td>
                    </tr>";
                }
                ?>
            </tbody>
